 ...
 */
interface StateValidationResult<T> {
  valid: boolean;
  data?: T;
  errors?: Array<{
    path: string;
...
/**
 * Enhanced Base Actor Class with AI-Friendly Features
 * 
 * New features:
 * 1. State validation and reconstruction
 * 2. AI-friendly error messages
 * 3. Enhanced lifecycle hooks
 * 4. Better testing utilities
 * 5. Development mode with introspection
 */
export abstract class EnhancedActor<TState extends ActorState = ActorState> {
  ...
  /**
   * Enhanced initialize with state validation and lifecycle hooks
   */
  async initialize(): Promise<void> {
    this.setLogContext('operation', 'initialize');
    this.log.info(`Initializing actor ${this.actorConfig.name}`);
    ...
    try {
      ...
      this.log.info(`Actor ${this.actorConfig.name} initialized successfully`);
    } catch (error) {
      throw new ActorError(
        ...
        'ACTOR_INIT_FAILED',
        {
          ...
          relatedFiles: [
            `${this.actorConfig.name}/src/actor.ts`,
            `${this.actorConfig.name}/src/state.ts`,
          ],
          helpfulCommands: [
...
protected async createDefaultState(): Promise<MyState> {
  ...
  protected async reconstructState(rawState: any): Promise<TState> {
    ...
    // If no schema, try to reconstruct Maps and Sets
    if (!this.stateSchema) {
      return this.reconstructComplexTypes(rawState);
    }
    
    // Validate with schema
    const validationResult = await this.validateState(rawState);
  ...
  private looksLikeMap(key: string): boolean {
    const mapIndicators = ['map', 'Map', 'dict', 'Dict', 'lookup', 'Lookup', 'byId', 'ById'];
        ...
        if (error instanceof z.ZodError) {
          throw new ActorError(
            ...
            'CONFIG_VALIDATION_FAILED',
            {
              ...
              relatedFiles: [`${this.actorConfig.name}/actor.config.json`],
  ...
  /**
   * Enhanced command handling with better error context
   */
      ...
      if (!validationResult.valid) {
        throw new ActorError(
          ...
          'COMMAND_VALIDATION_FAILED',
          {
            ...
            relatedFiles: [
              `${this.actorConfig.name}/src/events.ts`,
              `${this.actorConfig.name}/src/actor.ts`,
            ],
            helpfulCommands: [
...
await actor.handle({
  ...
  getIntrospection(): any {
    return {
      actor: this.actorConfig.name,
      version: this.actorConfig.version || '1.0.0',
  ...
  // ... (rest of the base Actor methods remain the same)
  ...
  async query(query: Query): Promise<QueryResult> {
    ...
    this.setLogContext('operation', 'query');
    this.setLogContext('queryType', query.type);
    this.setLogContext('correlationId', correlationId);
    ...
    try {
      ...
      if (!validationResult.valid) {
        throw new ActorError(
          ...
          'QUERY_VALIDATION_FAILED',
          {
            actor: this.actorConfig.name,
            query: query.type,
            fix: `Query payload validation failed:\n${validationResult.errors.join('\n')}`,
            relatedFiles: [
              `${this.actorConfig.name}/src/events.ts`,
              `${this.actorConfig.name}/src/actor.ts`,
            ],
            helpfulCommands: [
              `npm test -- --testNamePattern="${query.type}"`,
            ],
            documentation: `See query schema for ${query.type}`,
          },
          400,
          'Invalid request'
        );
      ...
      // After query hook
      await this.afterQuery(query, result);
      ...
      await this.onError(transformedError, { type: 'query', event: query });
      
      this.log.error(`Query ${query.type} failed`, transformedError, {
        duration,
        aiContext: transformedError instanceof ActorError ? transformedError.toAIFormat() : undefined,
      });
  ...
  /**
   * Save state with enhanced error handling
   */
      ...
      try {
        ...
        throw new ActorError(
          ...
          'STATE_SAVE_FAILED',
          {
            actor: this.actorConfig.name,
            state: 'persistence',
            fix: 'Check that the state persistence layer is available and that state can be serialized',
            helpfulCommands: [
              'npm test -- --testNamePattern="state"',
              'docker ps | grep redis', // If using Redis
            ],
            documentation: 'See state persistence documentation',
          },
          503,
  ...
  async performHealthCheck(): Promise<HealthStatus> {
    ...
    try {
      ...
      this.log.error('Health check failed', error as Error);
      return {
        healthy: false,
        checks: {
          error: {
  ...
  async shutdown(): Promise<void> {
    this.log.info(`Shutting down actor ${this.actorConfig.name}`);
    ...
    try {
      ...
      this.log.info(`Actor ${this.actorConfig.name} shut down`);
    } catch (error) {
  ...
  // Helper methods remain the same as base Actor...
  ...
  protected async publish(notification: Notification): Promise<void> {
    ...
    this.log.debug(`Published notification ${notification.type}`, {
  ...
  protected trackSecurityEvent(event: SecurityEventData): void {
    this.security.trackEvent({
      ...event,
      actorId: this.id,
      actorName: this.actorConfig.name,
    });
  ...
  on(eventType: string, handler: EventHandler): void {
    const wrappedHandler = async (event: Event) => {
      ...
      try {
        ...
        this.log.error(`Event handler for ${eventType} failed`, error as Error);
        throw error;
      } finally {
        timer();
      }
